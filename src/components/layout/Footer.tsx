...
// WhatsApp em formato wa.me (só dígitos: 55 + DDD + 9 + número).
...
const socials = [
  {
    Icon: IconInstagram,
    href: "https://www.instagram.com/solvydesenvolvimentos/",
    label: "Instagram",
  },
  {
    Icon: IconLinkedin,
    href: "https://www.linkedin.com/company/solvydesenvolvimentos",
    label: "LinkedIn",
  },
...
export default function Footer() {
  ...
  return (
    <footer className="relative isolate overflow-hidden rounded-t-[56px] border-t border-line bg-bg-2 text-fg">
      {/* Fundo: imagem igmfooter desfocada (efeito vidro fosco) + glow + véu */}
      <div aria-hidden className="pointer-events-none absolute inset-0 -z-10">
        ...
        />
        {/* painel de vidro fosco NA FRENTE da imagem (glass) */}
        <div className="absolute inset-0 bg-bg/25 backdrop-blur-[2px]" />
      </div>

      {/* Logo centralizada — só o wordmark "SOLVY" (SVG inline da Hero) */}
      <div className="flex justify-center px-6 pt-12 md:px-12 lg:px-20">
        <svg
          viewBox="350 25 1060 340"
      ...
      {/* Links + slogan */}
      <div className="flex flex-col gap-10 px-6 py-8 md:flex-row md:items-start md:justify-between md:px-12 lg:px-20">
        <nav className="flex flex-col items-center gap-4 md:-mt-8 md:items-start">
          {header.nav.map((l) => (
            <a
              key={l.href}
              href={l.href}
        ...
        <div className="display-tight text-center text-3xl font-semibold leading-tight tracking-tight text-fg md:text-right md:text-4xl">
          <p>SOFTWARE QUE</p>
          <p>CABE NA SUA</p>
          <p>EMPRESA.</p>
        </div>
      ...
      {/* Painel GLASS — vai até o FIM do footer (igual ref), full-width, com o
          crédito dentro. Mobile: ícone (em cima) · studio · redes, centralizado.
          Desktop: estúdio (esq) · ícone (centro) · redes (dir). */}
      <div className="border-t border-line bg-white/[0.05] backdrop-blur-md md:bg-white/[0.03]">
        <div className="flex flex-col items-center gap-6 px-6 py-8 md:grid md:grid-cols-3 md:items-center md:gap-8 md:px-12 md:py-7 lg:px-20">
          {/* estúdio + direitos (duas linhas) */}
          <div className="order-2 text-center md:order-none md:justify-self-start md:text-left">
            <p className="display-tight text-2xl font-semibold tracking-tight text-fg">
              2026 SOLVY STUDIO
            </p>
            <p className="mt-1 text-xs text-fg">{footer.rights}</p>
          </div>

          {/* ícone Solvy — PRIMEIRO no mobile (acima do studio); centro no desktop */}
          <svg
            viewBox="0 0 320 384"
            className="order-1 h-12 w-auto md:order-none md:justify-self-center"
            fill="#ffffff"
            aria-hidden="true"
          >
            <path d={ICON_A} />
          ...
          {/* redes */}
          <div className="order-3 flex items-center gap-3 md:order-none md:justify-self-end">
